...
public class DamerauLevenshteinDistance {
	...
	public static int distance(String s1, String s2) {
		log.info("Calculating Damerau-Levenshtein distance between '{}' and '{}'", s1, s2);
		int length1 = s1.length();
		int length2 = s2.length();
		int[][] dl = new int[length1 + 1][length2 + 1];
		log.debug("Initializing matrix with size ({}, {})", length1 + 1, length2 + 1);

		for (int i = 0; i <= length1; i++) {
			dl[i][0] = i;
		}
		log.debug("First column initialized");

		for (int j = 0; j <= length2; j++) {
			dl[0][j] = j;
		}
		log.debug("First row initialized");
		...
		log.debug("Distance result: {}", dl[length1][length2]);
