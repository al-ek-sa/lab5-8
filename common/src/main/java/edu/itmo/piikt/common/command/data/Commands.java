...
public enum Commands {
    HELP("help", "display help on available commands",
            "Both in the console and in the script, the command is entered as a single word without arguments. (\"help\")", false),

    INFO("info",
            "output information about the collection to stdout (type, initialization date, number of elements, etc.)",
            "Both in the console and in the script, the command is entered as a single word without arguments. (\"info\")", false),
    ...
    CLEAR("clear", "clear the collection",
            "Both in the console and in the script, the command is entered as a single word without arguments.(\"clear\")", false),

    EXECUTE_SCRIPT("execute_script", "read and execute a script from the specified file",
            "Both in the console and in the script, the command is entered on one line (\"execute_script _____\").", true),

    EXIT("exit", "terminate the program (without saving to file)",
            "Both in the console and in the script, the command is entered as a single word without arguments.(\"exit\")", false),

    HEAD("head", "output the first element of the collection",
            "Both in the console and in the script, the command is entered as a single word without arguments. (\"head\")", false),

    REMOVE_LOWER("remove_lower", "remove all elements from the collection that are less than the specified one",
            "Both in the console and in the script, the command is entered with a date (\"remove_lower yyyy-MM-dd\")", true),

    HISTORY("history", "output the last 14 commands (without their arguments)",
            "Both in the console and in the script, the command is entered as a single word without arguments. (\"history\")", false),
    ...
    FILTER_CONTAINS_NAME("filter_contains_name",
            ...
            "Both in the console and in the script, the command is entered, and then the employee's name is entered on the same line. (\"filter_contains_name _____\").", true),
    ...
    PRINT_FIELD_DESCENDING_END_DATE("print_field_descending_end_date",
            ...
            "Both in the console and in the script, the command is entered as a single \nword without arguments.(\"print_field_descending_end_date\")", false),

    HELP_ENTERING_COMMAND("help_entering_command", "display help on entering available commands",
            "Both in the console and in the script, the command is entered as a single word without arguments. (\"help_entering_command\")", false);
