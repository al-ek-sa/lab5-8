	...
	public void saveCollection() throws IOException {
		try (Context ignored = Context.newId()) {
			...
			try (PrintWriter writer = new PrintWriter(new FileWriter(fileName))) {
				...
				throw new IOException("Cannot save to file: " + fileName, e);
			} catch (Exception e) {
				logger.error("Error saving CSV: {}", e.getMessage());
				throw new IOException("Failed to save collection to CSV", e);
			}
		} catch (Exception e) {
			logger.error("Error in saveCollection: {}", e.getMessage());
			throw new IOException("Unexpected error during save", e);
		}
	}
	...
	public void readFile() throws IOException {
		try (Context ignored = Context.newId()) {
			...
			try (BufferedInputStream input = new BufferedInputStream(new FileInputStream(fileName));
				...
				throw new IOException("Failed to read collection from CSV", e);
			}
		} catch (Exception e) {
